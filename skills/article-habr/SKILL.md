...
name: article-habr
description: >-
  Adapts a finished article for Habr by selecting thematic hubs (from a bundled
  registry of 431 Habr hubs) and tags through a resumable interactive flow, then
  applies them into the index.md frontmatter (tags/hubs fields). Use after
...
Select Habr **hubs** (from a bundled registry) and **tags** for a finished
article through a resumable interactive dialogue, then write them into the
`tags`/`hubs` frontmatter of `index.md`. The LLM asks the human-facing
questions and proposes candidates; scripts own registry validation, state
persistence, and frontmatter edits.
...
| `--field <hubs\|tags>` | (`habr-answer`) which selection to save |
| `--value <list>` | (`habr-answer`) comma/semicolon/newline-separated hubs (titles or aliases) or tags |
| `--force` | (`habr-apply`) overwrite non-empty `tags`/`hubs` in `index.md` instead of writing `index.md.new` |
...
`habr-apply.mjs` writes the `tags` and `hubs` lines of the `index.md`
frontmatter, preserving the rest of the file:
...
indicators). If `index.md` already has non-empty `tags`/`hubs` that differ
from the new selection, `habr-apply.mjs` writes `index.md.new` instead of
overwriting unless `--force` is passed. If the frontmatter has no `tags`/`hubs`
lines, they are inserted; if `index.md` has no frontmatter at all, a new block
is prepended.
...
- `needs_hubs` — `cursor.phase === "hubs"`; `articleContext` carries the
  title, a lead excerpt, and the brief (topic/goal/audience/readerTakeaway/
  publicationTargets) so the agent can propose candidates. `registry` carries
  `registryPath` — read the full file to pick candidates.
- `needs_tags` — `cursor.phase === "tags"`; propose lowercase tags.
- `needs_apply` — both hubs and tags are chosen but not applied; run
  `habr-apply.mjs`.
- `all_done` — hubs and tags are already applied to `index.md`.

`habr-status.mjs --json` also returns `indexFrontmatter` with the hubs/tags
currently present in `index.md`, so the agent can detect drift between state
and the file.

## Agent instructions

When the user asks to prepare an article for Habr (pick hubs/tags):
...
6. For `needs_hubs`: read the registry at `registry.registryPath`
   (`assets/habr-hubs.json`). Using `articleContext` (title, lead excerpt,
   brief) as the signal, propose up to `maxHubs` candidate hubs that match the
   article topic. **Prefer hubs with `multiauthor: true`** (marked `*` on
   Habr) — only those accept posts from a regular author. Show the candidates
   with a note on which ones accept posts, and ask the user to confirm or
   edit the list (one question). Do **not** pick hubs blindly; a wrong hub
   means the article cannot be published there.
7. Save the confirmed hubs with `habr-answer.mjs --field hubs --value
   "Title 1, Title 2, …" --json`. Titles or aliases from the registry are
   accepted; unknown hubs are rejected with an error.
...
9. After both selections are saved, run `habr-apply.mjs --target . --slug
...
- Respect the user's wording when they give explicit hubs/tags; do not silently
  rewrite them. If a user-provided hub is not in the registry, tell them and
  suggest the closest match instead of failing silently.
